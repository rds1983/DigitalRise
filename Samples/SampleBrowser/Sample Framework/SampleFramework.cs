...
namespace Samples
{
	...
	public class SampleFramework
	{
		...
		private VerticalStackPanel _infoPanel;
		private Label _controllerLabel;
		private VerticalStackPanel _buttonsPanel;
		private VerticalStackPanel _fpsPanel;
		private Label _updateFpsLabel;
		private Label _drawFpsLabel;
		...
		public bool IsMenuVisible
		{
			get
			{
				return _menuWindow.IsPlaced
					   || _profilerWindow.IsPlaced
					   || _optionsWindow.IsPlaced
					   || _helpWindow.IsPlaced;
			}
		}
      ...
      _inputService.SetLogicalPlayer(LogicalPlayerIndex.One, PlayerIndex.One);
		...
		private void InitializeGui()
		{
			...
			_guiGraphicsScreen.UIScreen.Widgets.Add(_titleLabel);

			// ----- Info (bottom left)
			_infoPanel = new VerticalStackPanel
			{
				Margin = new Thickness(10),
				HorizontalAlignment = HorizontalAlignment.Left,
				VerticalAlignment = VerticalAlignment.Bottom,
			};
			_guiGraphicsScreen.UIScreen.Widgets.Add(_infoPanel);
			_controllerLabel = new Label
			{
				TextColor = Color.White,
				Text = "/es1Controller disconnected. Press <Start> to use controller.",
			};
			_infoPanel.Widgets.Add(_controllerLabel);

			// ----- Buttons (bottom right)
			_buttonsPanel = new VerticalStackPanel
			{
				Background = new SolidBrush(new Color(0, 0, 0, 64)),
				Margin = new Thickness(10),
				HorizontalAlignment = HorizontalAlignment.Right,
				VerticalAlignment = VerticalAlignment.Bottom,
			};
			_guiGraphicsScreen.UIScreen.Widgets.Add(_buttonsPanel);
			...
			_fpsPanel = new VerticalStackPanel
			{
				Margin = new Thickness(10),
				HorizontalAlignment = HorizontalAlignment.Right,
				VerticalAlignment = VerticalAlignment.Top,
			};
			_guiGraphicsScreen.UIScreen.Widgets.Add(_fpsPanel);
			_updateFpsLabel = new Label
			{
				TextColor = Color.Yellow,
				HorizontalAlignment = HorizontalAlignment.Right,
				Text = "/es1Update",
			};
			_fpsPanel.Widgets.Add(_updateFpsLabel);
			_drawFpsLabel = new Label
			{
				TextColor = Color.Yellow,
				HorizontalAlignment = HorizontalAlignment.Right,
				Text = "/es1Draw",
			};
			_fpsPanel.Widgets.Add(_drawFpsLabel);
		...
		private void AddButton(StackPanel panel, string text, Action clickHandler)
		{
			...
			if (panel.Widgets.Count > 0)
				button.Margin = (panel.Orientation == Orientation.Horizontal) ? new Thickness(3, 0, 0, 0) : new Thickness(0, 3, 0, 0);
      ...
      button.Margin = new Thickness(15);
		...
		private void CreateMenuWindow()
		{
			...
			var tabControl = new TabControl
			{
				...
				TabSelectorPosition = TabSelectorPosition.Left,
				Width = 580,
#if WINDOWS_PHONE || ANDROID || IOS
        Height = 300
#else
			...
			foreach (var grouping in samplesByCategory)
			{
				...
				var itemsPanel = new VerticalStackPanel
				{
				...
				{
					var item = new VerticalStackPanel();
		...
		private void CreateHelpWindow()
		{
			...
			_helpLabel = new Label(); // Text is set in UpdateHelpText().
		...
		public void ShowOptionsWindow(string tabItem)
		{
			...
			var tab = _optionsTabControl.Items.FirstOrDefault(t => t.Text == tabItem);
			if (tab != null)
			{
				_optionsTabControl.SelectedItem = tab;
			}
		...
		private void UpdateController()
		{
			...
			_controllerLabel.Visible = !controllerIsConnected;
#endif
		}
			...
			{
				{
					...
					_updateFpsLabel.Text = _stringBuilder.ToString();
				}
				{
					_stringBuilder.Clear();
					...
					_stringBuilder.AppendNumber(1 / fps * 1000, 2, AppendNumberOptions.None);
					_stringBuilder.Append(" ms");
					_drawFpsLabel.Text = _stringBuilder.ToString();
				}
      ...
      foreach (var hierarchicalProfiler in hierarchicalProfilers)
      {
        ...
        hierarchicalProfiler.Reset();
			...
			_profilerLabel.Text = _stringBuilder.ToString();
		}
		...
		private void ResetOptions()
		{
			for (int i = _optionsTabControl.Items.Count - 1; i >= 0; i--)
				if (_optionsTabControl.Items[i].Text != "General")
		...
		private void UpdateHelp()
		{
			...
			_titleLabel.Text = "/es1";

			_titleLabel.Text += (_sample != null) ? category + " - " + name : name;
