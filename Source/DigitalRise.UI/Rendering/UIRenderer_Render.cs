	...
	public partial class UIRenderer
	{
		...
		public void RenderImage(RectangleF bounds, ThemeImage image, float opacity, UIRenderContext context)
		{
			...
			if (image.HorizontalAlignment == HorizontalAlignment.Stretch || image.VerticalAlignment == VerticalAlignment.Stretch)
			{
				...
				RenderStretchedImage(image, bounds, context, color);
			}
			else
			{
				// Draw a non-stretched image.
				RenderImage(image, bounds, context, color);
			}
		}
		...
		private void RenderStretchedImage(ThemeImage image, RectangleF area, UIRenderContext context, Color color)
		{
			var region = (NinePatchRegion)image.TextureRegion;
			var texture = region.Texture;
			Rectangle source = region.Rectangle;
			int left = region.Border.Left;
			int top = region.Border.Top;
			int right = region.Border.Right;
			int bottom = region.Border.Bottom;
		...
		/// <param name="color">The tint color.</param>
		private void RenderImage(ThemeImage image, RectangleF area, UIRenderContext context, Color color)
		{
			var horizontalAlignment = image.HorizontalAlignment;
			var verticalAlignment = image.VerticalAlignment;
			var tileMode = image.TileMode;
			...
			var source = image.TextureRegion.Rectangle;
			...
			var texture = image.TextureRegion.Texture;
			switch (tileMode)
			{
				case TileMode.None:
