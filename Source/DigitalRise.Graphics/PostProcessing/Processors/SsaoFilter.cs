...
namespace DigitalRise.Graphics.PostProcessing
{
	...
	public class SsaoFilter : PostProcessor
	{
		...
		{
			_effect = GraphicsService.GetStockEffect("DigitalRise/PostProcessing/SsaoFilter");
			...
			for (int i = 0; i < vectors.Length; i++)
				vectors[i] = (Vector3)(Matrix33F.CreateRotationZ(ConstantsF.TwoPi * i / vectors.Length + random.NextFloat(-jitterAngle, jitterAngle)) * new Vector3(1, 0, 0)).Normalized();
