	...
	public partial class Ephemeris
	{
      ...
      Time = new DateTimeOffset(DateTime.Now.Ticks, TimeSpan.Zero);
		...
		public void Update()
		{
			...
			EclipticToEquatorial = Matrix33F.CreateRotationX((float)_e);
			...
			EquatorialToGeographic = Matrix33F.CreateRotationZ((float)-gmst);
			...
			// In original article:
			var Ry = Matrix33F.CreateRotationY((float)(-0.00972 * _epoch2000Centuries));
			var Rz = Matrix33F.CreateRotationZ((float)(0.01118 * _epoch2000Centuries));
			...
			var rLong = Matrix33F.CreateRotationZ((float)-lmst);
			...
			var equatorialToHorizontalTranslation = new Vector3(0, (float)(-earthRadius - Altitude), 0);
		...
		{
			double Np = ((2.0 * ConstantsF.Pi) / 365.242191) * (_epoch1990Days / (float)_planetElements[(int)VisiblePlanets.Earth].Period);
			Np = InRange(Np);
