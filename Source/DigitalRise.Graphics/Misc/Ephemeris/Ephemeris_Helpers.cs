...
namespace DigitalRise.Graphics
{
	partial class Ephemeris
	{
		...
		/// </remarks>
		private static Vector3 ToCartesian(double radius, double latitude, double longitude)
		{
			...
			Vector3 v;
			v.X = (float)(radius * cosLong * cosLat);
			v.Y = (float)(radius * sinLong * cosLat);  // East
			v.Z = (float)(radius * sinLat);            // Up
			return v;
		}
		...
		private static double Refract(double elevation)
		{
			...
			else
			{
				...
				{
					double degElev = MathHelper.ToDegrees((float)elevation);
			...
			return elevation + MathHelper.ToRadians((float)refcor);
		}
	}
}
